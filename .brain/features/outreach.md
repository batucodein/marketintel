...
## HTTP surface (P1 delivered so far)
...
Still TODO for P1:
- `GET /outreach/conversations` — inbox
- `GET /outreach/conversations/{id}` — thread detail
- `POST /outreach/conversations` — start conversation with a contact
- `POST /outreach/conversations/{id}/messages` — send a reply
...
**Backend scaffolding ~60% of P1.** Fully operational for channel CRUD and OAuth round-trip; receiving is implemented but not yet wired to a poller goroutine. Send works at the channel level but there is no handler endpoint yet (awaiting conversation service).

**Blockers for end-to-end test:**
- Need `GMAIL_CLIENT_ID` and `GMAIL_CLIENT_SECRET` from GCP console
- Need `TOKEN_ENCRYPTION_KEY` (can be generated locally via `openssl rand -base64 32`)

**Frontend: not started yet for outreach.**
